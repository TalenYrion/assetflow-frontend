    ...
    return (
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="animate-pulse rounded-xl border border-neutral-200 bg-white p-4 h-80 dark:border-neutral-800 dark:bg-neutral-950"
          />
        ))}
      </div>
    );
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {assets.map((asset) => (
          ...
          >
            ...
            <Link href={`/browse/${asset.id}`} className="flex flex-col flex-1">
              <div className="relative aspect-video w-full bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center overflow-hidden border-b border-neutral-100 dark:border-neutral-900">
                {asset.thumbnail?.url ? (
                  ...
                  <div className="flex flex-col items-center gap-2 text-neutral-400 dark:text-neutral-600">
                    ...
                    <span className="text-[10px] font-mono tracking-wider uppercase font-semibold">No Preview</span>
                  </div>
                )}
                <span className="absolute bottom-2 right-2 rounded bg-neutral-900/80 px-1.5 py-0.5 font-mono text-[10px] font-semibold uppercase text-white backdrop-blur-xs">
                  {asset.fileExtension}
                </span>
              </div>
              ...
              <div className="flex flex-col p-4 space-y-1.5">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-sm font-semibold truncate text-neutral-900 dark:text-neutral-50 group-hover:text-blue-500 transition">
                    {asset.title}
                  </h3>
            ...
            <div className="p-4 pt-0 border-t border-neutral-100 dark:border-neutral-900/60 space-y-3 mt-auto">
              <Link 
                href={`/creator/${asset.creator.id}`}
                className="flex items-center gap-2 pt-3 hover:opacity-80 transition max-w-max"
              >
                <div className="h-5 w-5 overflow-hidden rounded-full bg-neutral-200 shrink-0">
                ...
                <span className="text-[11px] font-medium text-neutral-600 dark:text-neutral-400 hover:underline truncate">
